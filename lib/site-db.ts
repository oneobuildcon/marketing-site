...
// ── Quotation package specifications ────────────────────────────────────────
// The admin can edit a package's specs, rates, brands, notes, payment schedule
// and area percentages, then save them. Saved overrides are stored per package
// id and take the place of the code defaults in data/quotationSpecs.ts. Only
// the fields actually saved are stored, so a package the admin never edited
// keeps following the code defaults.
export type QuotationOverride = {
  rate?: number;
  sections?: unknown;
  rates?: unknown;
  brands?: unknown;
  notes?: unknown;
  payments?: unknown;
  areaPercents?: unknown;
};
export type QuotationOverrides = Record<string, QuotationOverride>;

export async function getQuotationOverrides(): Promise<QuotationOverrides> {
  if (!hasSupabase()) return {};
  try {
    const supabase = createServerClient();
    const { data, error } = await supabase
      .from('site_settings')
      .select('value')
      .eq('key', 'quotationSpecs')
      .maybeSingle();
    if (error) throw error;
    const v = data?.value as QuotationOverrides | undefined;
    return v && typeof v === 'object' ? v : {};
  } catch (e) {
    console.error('getQuotationOverrides: falling back to code defaults:', e);
    return {};
  }
}

export async function saveQuotationOverrides(value: QuotationOverrides): Promise<void> {
  if (!hasSupabase()) throw new Error('Database not configured');
  const supabase = createServerClient();
  const { error } = await supabase
    .from('site_settings')
