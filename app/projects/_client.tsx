...
import type { ProjectCategory, Project } from "@/lib/projects";
...
export default function ProjectsClient({ projects }: { projects: DbProject[] }) {
        ...
        <div className="mx-auto max-w-6xl px-6">
          <motion.div layout className="grid gap-8 sm:grid-cols-2 lg:grid-cols-3">
            {filtered.map((project) => {
              ...
              const cover = project.photos?.[0] ?? `/projects/${project.slug}/1.jpg`;
              return (
                <MotionLink
                  key={project.slug}
                ...
                >
                  <div className="relative h-52 w-full overflow-hidden">
                    <Image src={cover} alt={c.name} fill className="object-cover transition-transform duration-700 group-hover:scale-110" />
                    ...
                    {photoCount > 1 && (
                      <span className="absolute right-3 bottom-3 rounded-full bg-navy/80 px-2.5 py-1 text-xs font-semibold text-white flex items-center gap-1 backdrop-blur-sm">
                        <Images className="h-3.5 w-3.5 text-amber" /> {photoCount} {t.photos}
                      </span>
                    )}
