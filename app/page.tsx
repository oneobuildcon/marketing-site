...
const translations = {
  en: {
    ...
    stats: [
      ...
      { label: "Sq.ft Constructed",  value: 67000, suffix: "+" },
  ...
  mr: {
    ...
    stats: [
      ...
      { label: "बांधलेले चौ.फूट",       value: 67000, suffix: "+" },
...
export default function Home() {
  ...
  const statValues = liveStats ? [liveStats.projects, liveStats.years, liveStats.sqft || 67000, liveStats.cities] : null;
    ...
    <main>
      ...
      <section className="relative overflow-hidden bg-navy text-white" style={{ minHeight: "90vh", display: "flex", alignItems: "center" }}>
        <motion.div className="absolute inset-0" style={{ y: heroImgY }}>
          <Image src="/hero.jpg" alt="Completed One O Buildcon bungalow in Charoli, Pune" fill priority sizes="100vw" quality={70} className="object-cover opacity-30 scale-110" />
        </motion.div>
              ...
              <div className="grid grid-cols-2 gap-3">
                {displayStats.map((stat) => (
                  <div key={stat.label} className="rounded-xl bg-white/5 border border-white/10 px-4 py-3 backdrop-blur-sm">
                    <p className="text-2xl font-black text-amber">{typeof stat.value === "number" ? stat.value.toLocaleString("en-IN") : stat.value}{stat.suffix}</p>
