...
type PackagesData = { content: PackageContent; pkgMeta: PackageMeta[]; catMeta: CategoryMeta[] };

export default function Packages({ initial }: { initial: PackagesData }) {
  const { lang } = useLanguage();
  const ui = uiTranslations[lang];
  // Initialise from the server-rendered live data so the correct prices/items
  // show on first paint — no flash of the bundled defaults.
  const [content, setContent] = useState<PackageContent>(initial.content ?? defaultPackageContent);
  const [pkgMeta, setPkgMeta] = useState<PackageMeta[]>(initial.pkgMeta?.length ? initial.pkgMeta : defaultPkgs.map((p) => ({ ...p })));
  const [catMeta, setCatMeta] = useState(initial.catMeta?.length ? initial.catMeta : categories.map((c) => ({ ...c })));
  const [selectedPkg, setSelectedPkg] = useState<PackageId>(
    initial.pkgMeta?.some((p) => p.id === "basic") || !initial.pkgMeta?.length ? "basic" : initial.pkgMeta[0].id
  );
  const [selectedCat, setSelectedCat] = useState<CategoryId>(
    (initial.catMeta?.some((c) => c.id === "designs") || !initial.catMeta?.length ? "designs" : initial.catMeta[0].id) as CategoryId
  );
